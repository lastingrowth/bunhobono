  ...
  <Teleport to="body">
    <Transition name="delete-modal">
      <div v-if="open" class="delete-modal-backdrop" @click.self="$emit('cancel')">
        <section class="delete-modal" role="dialog" aria-modal="true" aria-labelledby="notice-delete-title">
          <div class="delete-modal-icon" aria-hidden="true">!</div>
          <h2 id="notice-delete-title">알림 기록 삭제</h2>
          <p>
            <strong>{{ carNo || "차량번호 없음" }}</strong> 차량의 알림 기록을<br>
            삭제하시겠습니까?
          </p>
          <small>삭제된 알림 기록은 복원할 수 없습니다.</small>

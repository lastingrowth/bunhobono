-- bono_demo_reset.sql 기반 통합 리셋 더미
-- 변경 범위: apartment_unit, member, member_archive 및 새 회원 PK 구조에 필요한 FK 보정
-- 그 외 시연 데이터는 원본 bono_demo_reset.sql을 유지한다.
BEGIN;
...
ALTER TABLE notice
    ...
    DROP CONSTRAINT IF EXISTS chk_notice_source;
    ...
    ADD CONSTRAINT chk_notice_source
        CHECK (
            (
                ...
                AND (
                    car_log_no IS NOT NULL
                    OR snapshot_car_log_no IS NOT NULL
                )
            )
            OR
            (
...
DROP INDEX IF EXISTS uq_notice_type_car_log;
DROP INDEX IF EXISTS uq_notice_type_camera;
...
-- 402동 전체와 실제 회원이 없는 일부 세대는 EMPTY로 유지
-- ====================================-- 실제 아파트 세대 160개를 먼저 생성한다.
...
-- 기존 더미의 회원 번호를 유지하기 위한 임시 적재 테이블이다.
-- EMPTY 가상 회원은 member에 넣지 않고 apartment_unit의 EMPTY 상태로 표현한다.
-- =================


CREATE TEMP TABLE legacy_member_seed (
    seed_order BIGSERIAL PRIMARY KEY,
    login_id VARCHAR(30),
...
SELECT
    ...
    seed.login_id,
    seed.login_pwd,
    CASE WHEN seed.role = 'ADMIN' THEN NULL ELSE au.unit_no END,
    seed.mem_name,
    seed.mem_phone,
    seed.role,
    seed.mem_status,
...
VALUES
    ...
    ('normal',  '55오0359',  'APPROVED', CURRENT_TIMESTAMP - INTERVAL '3 hours', CURRENT_TIMESTAMP + INTERVAL '21 hours', 95, CURRENT_TIMESTAMP - INTERVAL '4 hours'),
...
-- =====================================================
-- 8. 관리자 알림 View
-- 현재 NoticeMapper가 조회하는 두 View를 초기화 SQL에도 포함한다.
-- =====================================================
...
-- =====================================================
-- 8-1. 방문차량·미등록차량 초과 알림
-- 미출차/출차완료, 미처리/처리완료를 모두 시연할 수 있게 구성한다.
-- =====================================================
INSERT INTO notice (
    notice_type,
    car_log_no,

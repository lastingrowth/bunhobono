...
STREAM_FRAME_DIR = BASE_DIR / "runtime" / "stream_frames"
STREAM_CROP_DIR = BASE_DIR / "runtime" / "stream_crops"
STREAM_CANDIDATE_DIR = BASE_DIR / "runtime" / "stream_candidates"
...
class CameraStreamWorker:
    def __init__(self, camera, detector, ocr_reader, model_lock):
    ...
    def run(self):
        ...
        print(f"[{self.camera_name}] 영상 시작: {self.source}")
        ...
        while True:
            ...
            if not ret:
                print(f"[{self.camera_name}] 영상 끝. 처음부터 다시 재생")
    ...
    # 프레임 1장에서 번호판 crop 후보 1개를 만들고 OCR 처리
    ...
    # crop 후보 5개 중 OCR 신뢰도가 가장 높은 1개 선택
    def finish_candidates(self):
        ...
        self.last_send_time[car_no] = time.time()

    # 후보 crop 5개를 테스트 확인용으로 저장
    ...
    # 같은 차량번호를 너무 자주 보내지 않도록 막음
    def is_cooldown(self, car_no):
        last_time = self.last_send_time.get(car_no)
    ...
    def save_original_frame(self, frame, car_no, frame_no):
        filename = f"{self.camera_name}_{car_no}_frame{frame_no}_{int(time.time())}.jpg"
    ...
    def save_best_crop(self, crop_path, car_no, frame_no):
        src = Path(crop_path)
        filename = f"{self.camera_name}_{car_no}_frame{frame_no}_best_crop.jpg"
...
def main():
    ...
    for camera in CAMERAS:
        print(
            f"카메라 등록: cameraNo={camera['cameraNo']}, "
            f"name={camera['name']}, source={camera['source']}"
        )
...
# C:\Users\dkddp\.conda\envs\bono\python.exe camera_stream.py
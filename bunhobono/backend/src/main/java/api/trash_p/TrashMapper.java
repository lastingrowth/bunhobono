...
public interface TrashMapper {
    ...
    // 정산서와 조회에 필요한 연결 정보를 지난 기록에 저장
    @Insert("INSERT INTO trash_bin " +
            " (data_type, original_no, data_json, delete_type) " +
            " SELECT 'BILL', bill_no, TO_JSONB(bill_detail), #{deleteType} " +
            " FROM bill_detail " +
    ...
    @Insert("INSERT INTO " +
            " bill (bill_no, car_log_no, snapshot_car_log_no, " +
            " snapshot_car_no, fee_rule_no, " +
            " kiosk_no, charge_minutes, bill_amount, bill_status, " +
            " payment_order_id, payment_key, payment_method, issued_at, paid_at) " +
            " SELECT " +
            "    (data_json ->> 'bill_no')::INT, " +
            "    CASE " +
            "        WHEN EXISTS (" +
            "            SELECT 1 " +
            "            FROM car_log " +
            "            WHERE car_log_no = NULLIF(data_json ->> 'snapshot_car_log_no', '')::INT" +
            "        ) " +
            "        THEN NULLIF(data_json ->> 'snapshot_car_log_no', '')::INT " +
            "        ELSE NULL " +
            "    END, " +
            "    (data_json ->> 'snapshot_car_log_no')::INT, " +
            "    data_json ->> 'snapshot_car_no', " +
            "    (data_json ->> 'fee_rule_no')::INT, " +
            "    CASE " +
            "        WHEN EXISTS (" +
            "            SELECT 1 " +
            "            FROM kiosk " +
            "            WHERE kiosk_no = " +
            "                NULLIF(data_json ->> 'kiosk_no', '')::INT " +
            "        ) " +
            "        THEN NULLIF(data_json ->> 'kiosk_no', '')::INT " +
            "        ELSE NULL " +
            "    END, " +
            "    (data_json ->> 'charge_minutes')::INT, " +
            "    (data_json ->> 'bill_amount')::NUMERIC, " +
            "    data_json ->> 'bill_status', " +
            "    data_json ->> 'payment_order_id', " +
            "    data_json ->> 'payment_key', " +
            "    data_json ->> 'payment_method', " +
            "    (data_json ->> 'issued_at')::TIMESTAMP, " +
            "    NULLIF(data_json ->> 'paid_at', '')::TIMESTAMP " +
            " FROM trash_bin " +
            " WHERE trash_no = #{trashNo} " +
            "   AND data_type = 'BILL'")
    int restoreBill(@Param("trashNo") int trashNo);

...
public class CarLogService {
    ...
    // 입차 기록 생성값 구성
    private CarLogDTO createEntryLog(CameraDataDTO cameraData, int gateNo) {
        CarLogDTO log = new CarLogDTO();

        log.setVehicleCarNo(cameraData.getVehicleCarNo());
        log.setCameraDataNo(cameraData.getCameraDataNo());
        log.setInGateNo(gateNo);
        log.setInTime(captureTime(cameraData));

        boolean isVisitVehicle = "visit".equalsIgnoreCase(cameraData.getVehicleType());
        boolean isResidentVisit = isVisitVehicle && cameraData.getApartmentUnitNo() != null;

        // 입주민 방문차량은 등록할 때 선택한 이용시간만큼 무료,
        // 긴급·작업 방문차량은 72시간 무료,
        // 관리실 일반 방문차량과 그 외 차량은 무료시간이 없다.
        if (isResidentVisit) {
            log.setFreeTime(calculateResidentVisitFreeMinutes(cameraData));
        } else if (isEmergencyVisit(cameraData)) {
            log.setFreeTime(4320);
        } else {
            log.setFreeTime(0);
        }

        log.setSnapshotCarNo(resolvedCarNo(cameraData));
    ...
    private boolean isEmergencyVisit(CameraDataDTO cameraData) {
        return cameraData != null
                ...
                && cameraData.getEndDate().equals(cameraData.getStartDate().plusHours(72))
                && isWithinVisitPeriod(cameraData);
    }

    // 입차 기록이 긴급·작업 차량인지 확인한다.
    private boolean isEmergencyLog(CarLogDTO log) {
        return log != null
                && "VISIT".equalsIgnoreCase(log.getSnapshotCarKind())
                && Integer.valueOf(4320).equals(log.getFreeTime());
    }

    // 방문 시작·종료시각의 차이를 입차 기록의 무료시간으로 사용한다.
    private int calculateResidentVisitFreeMinutes(CameraDataDTO cameraData) {
        if (cameraData.getStartDate() == null || cameraData.getEndDate() == null
                || !cameraData.getEndDate().isAfter(cameraData.getStartDate())) {
        ...
        if (freeMinutes < 60 || freeMinutes > 24 * 60 || freeMinutes % 60 != 0) {
            throw new ResponseStatusException(
                    HttpStatus.CONFLICT,
